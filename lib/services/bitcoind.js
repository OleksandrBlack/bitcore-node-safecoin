...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  $.checkArgument(this.options.spawn, 'Please specify "spawn" in safecoind config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in safecoind config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in safecoind config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './safecoin.conf');

  log.info('Using safecoin config file:', configPath);
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to safecoind from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from safecoind. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from safecoind. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in safecoin.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that safecoind is undergoing a reindex. ' +
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('Safecoin Daemon Ready');
          callback();
        });
      });
...
Bitcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/safecoin.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/safecoin.conf';
    }
  }
  return networkPath;
};
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Safecoin Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Safecoin Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
...
Bitcoin.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/safecoind.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned safecoin process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean safecoin process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Bitcoin.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting safecoin process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Safecoin process unexpectedly exited with code:', code);
        log.warn('Restarting safecoin child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Safecoin process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn safecoind.'));
      }
...
Bitcoin.prototype._connectProcess = function(config, callback) {
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to safecoind.'));
    }
...
Bitcoin.prototype.start = function(callback) {
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Safecoin configuration options "spawn" or "connect" are expected'));
    }
  ...
  this.client.getInfo(function(err, response) {
    ...
    self.client.getNetworkInfo(function(err, netResponse) {
      ...
      var info = {
		...
		tls_connections: result.tls_connections,
        proxy: result.proxy,
        difficulty: result.difficulty,
  ...
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('safecoind spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('safecoind process did not exit'));
      }
...
var subsidy = new BN(0)
  	...
  	var subsidy = new BN(4000000 * 1e8)
  
} else if (height == 80185) {
  	var subsidy = new BN(665600 * 1e8) //refund for interest rate attack
  
} else if (height < 123840) {
  	var subsidy = new BN(128 * 1e8)

} else if (height == 176406) {
  	var subsidy = new BN(-25599744 * 1e8)
	
} else if (height < 178378) {
  	var subsidy = new BN(64 * 1e8)
  
} else if (height < 181378) {
  	var subsidy = new BN(56 * 1e8)
  
} else if (height < 184378) {
  	var subsidy = new BN(48 * 1e8)
  
} else if (height < 187378) {
  	var subsidy = new BN(40 * 1e8)
  
} else if (height < 197378) {
  	var subsidy = new BN(32 * 1e8)
  
} else if (height < 207378) {
  	var subsidy = new BN(28 * 1e8)
  
} else if (height < 217378) {
  	var subsidy = new BN(24 * 1e8)
  
} else if (height < 227378) {
  	var subsidy = new BN(22 * 1e8)
  
} else if (height < 237378) {
  	var subsidy = new BN(20 * 1e8)
  
} else if (height < 247378) {
  	var subsidy = new BN(18 * 1e8)
  
} else if (height < 287378) {
  	var subsidy = new BN(16 * 1e8)
  
} else if (height < 327378) {
  	var subsidy = new BN(15 * 1e8)
  
} else if (height < 367378) {
  	var subsidy = new BN(14 * 1e8)
  
} else if (height < 407378) {
  	var subsidy = new BN(13 * 1e8)
  
} else if (height < 447378) {
  	var subsidy = new BN(12 * 1e8)
  
} else if (height < 487378) {
  	var subsidy = new BN(11 * 1e8)
  	...
  	var subsidy = new BN(4 * 1e8)
  
} else if (height < 1527378) {
  	var subsidy = new BN(3 * 1e8)
  
} else if (height < 1847378) {
  	var subsidy = new BN(2 * 1e8)
  
} else if (height < 2167378) {
  	var subsidy = new BN(1 * 1e8)
  
} else if (height < 3447378) {
  	var subsidy = new BN(0.5 * 1e8)
  
} else if (height < 4727378) {
  	var subsidy = new BN(0.25 * 1e8)
