...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  var configPath = (spawnOptions['configPath']) ? path.resolve(spawnOptions.configPath) : path.resolve(spawnOptions.datadir, './safecoin.conf');
      ...
      return async.waterfall([
        function (callback) {
          ...
          return client.getRawTransaction(txid, 1, function(err, response) {
              ...
              if (size == null && result.hex) {
                size = result.hex.length / 2;
              } 
              var tx = {
                  hex: result.hex,
              ...
              if (tx.version == 5) {
                var ZELNODE_START_TX_TYPE = 2;
                var ZELNODE_CONFIRM_TX_TYPE = 4;
                tx.type = result.type;
                ...
                if (tx.nType == ZELNODE_CONFIRM_TX_TYPE) {
                  tx.ip = result.ip;
                  tx.updateType = result.update_type;
                  tx.benchmarkTier = result.benchmark_tier;
                  tx.benchmarkSigTime = result.benchmark_sigtime;
                  tx.benchmarkSig = result.benchmark_sig;
                }
              } else {
              tx.locktime = result.locktime;
              tx.fOverwintered = result.overwintered;
...
Bitcoin.prototype.listzelnodes = function(callback) {
  var self = this;
  this.client.listzelnodes(function(err, response) {
  if (err) {
    return callback(self._wrapRPCError(err));
  }
  var result = response;
...
Bitcoin.prototype.viewdeterministiczelnodelist = function(callback) {
  var self = this;
  this.client.viewdeterministiczelnodelist(function(err, response) {
  if (err) {
    return callback(self._wrapRPCError(err));
  }
  var result = response;
